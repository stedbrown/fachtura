...
export async function POST(request: NextRequest) {
  try {
    const body: QuotePreviewData = await request.json()
    const { locale = 'it', client, items, quote_number, date, valid_until, notes, subtotal, tax_amount, total } = body
    ...
    pdf.fontSize(10).font('Helvetica')
    const details = [
      `${t.quoteNumber}: ${quote_number}`,
      date ? `${t.date}: ${format(new Date(date), 'dd MMMM yyyy', { locale: dateLocale })}` : null,
      valid_until ? `${t.validUntil}: ${format(new Date(valid_until), 'dd MMMM yyyy', { locale: dateLocale })}` : null,
    ].filter(Boolean)
    ...
    if (company.quote_terms_conditions && yPosition < 650) {
      pdf.fontSize(10).font('Helvetica-Bold')
    ...
    // Footer Text
    if (company.quote_footer_text && yPosition < 650) {
