...
export default function EditInvoicePage() {
  ...
  const loadInvoice = async () => {
    try {
      ...
      setStatus((data.status as InvoiceStatus) || 'draft')
      setNotes(data.notes || '')
      const mappedItems: InvoiceItemInput[] = (data.invoice_items || []).map((item: any) => ({
      ...
      toast.error('Impossibile caricare la fattura')
      router.push(`/${locale}/dashboard/invoices`)
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(tCommon('saveSuccess') || 'Fattura aggiornata con successo')
      router.push(`/${locale}/dashboard/invoices`)
    } catch (error: any) {
      console.error('Error updating invoice:', error)
      toast.error(error?.message || 'Errore aggiornamento fattura')
    } finally {
      setLoading(false)
    }
  }

  const totals = calculateInvoiceTotals(items.length ? items : [{ description: '', quantity: 1, unit_price: 0, tax_rate: 8.1 }])
